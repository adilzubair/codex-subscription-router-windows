...
func New(address, token string, multiplexer *mux.Multiplexer, uiTests bool) *Server {
	...
	router.HandleFunc("/", server.manager)
	router.HandleFunc("/manager", server.manager)
	router.HandleFunc("/manager/assets/manager.css", server.managerAsset)
	router.HandleFunc("/manager/assets/manager.js", server.managerAsset)
...
func (s *Server) routing(response http.ResponseWriter, request *http.Request) {
	...
	case http.MethodPatch:
		...
		ctx, cancel := context.WithTimeout(request.Context(), 20*time.Second)
		defer cancel()
		preference, err := s.mux.SetRoutingPreference(ctx, input.AccountID)
		if err != nil {
			writeJSON(response, http.StatusBadRequest, map[string]any{"error": err.Error()})
			return
		}
